        ...
        private void parse_table(ftable_t tab, UInt32 num)
        {
            ...
            for (int i = 0; i <= tab.Table_Entry_Count; i++)
            {
                ...
                else
                {
                    ...
                    byte[] DecompOut = new byte[tab.Fentry_Ucsize];

                    List<byte> Temper = new List<byte>();
                    Temper.AddRange(TrimEnd(Decompressed));
                    
                    while(Temper.Count < tab.Fentry_Ucsize && tab.Fentry_Ucsize < 0x500000)
                    {
                        Temper.Add(00);
                    }

                    DecompOut = Temper.ToArray();
